import type ***REMOVED*** NoInfer ***REMOVED*** from '@d-fischer/shared-utils';
declare type PackageName = 'twitch' | 'twitch-auth' | 'twitch-chat-client' | 'twitch-eventsub' | 'twitch-pubsub-client' | 'twitch-webhooks';
export declare function rtfm(pkg: PackageName, name: string): ClassDecorator;
export declare function rtfm<T>(pkg: PackageName, name: string, idKey: keyof NoInfer<T>): ClassDecorator;
export ***REMOVED******REMOVED***;
