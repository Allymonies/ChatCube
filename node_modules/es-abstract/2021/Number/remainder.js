...
module.exports = function NumberRemainder(n, d) ***REMOVED***
	if (Type(n) !== 'Number' || Type(d) !== 'Number') ***REMOVED***
		throw new $TypeError('Assertion failed: `n` and `d` arguments must be Numbers');
	***REMOVED***

	// If either operand is NaN, the result is NaN.
	// If the dividend is an infinity, or the divisor is a zero, or both, the result is NaN.
	if (isNaN(n) || isNaN(d) || !isFinite(n) || d === 0) ***REMOVED***
		return NaN;
	***REMOVED***

	// If the dividend is finite and the divisor is an infinity, the result equals the dividend.
	// If the dividend is a zero and the divisor is nonzero and finite, the result is the same as the dividend.
	if (!isFinite(d) || n === 0) ***REMOVED***
		return n;
	***REMOVED***
